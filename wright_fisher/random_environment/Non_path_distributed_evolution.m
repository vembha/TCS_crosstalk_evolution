function [] = Non_path_distributed_evolution(N,mut,majority_percent)
...
while (gen<=10000)
    ...
    if (numb(fixation,gen)>=majority*total_bact)
        break;
    end
...
%% SAVING THE RESULTS
save(['Non_path_distributed_evolution_N_',num2str(N),'_mut_',num2str(mut),'_maj_',num2str(majority_percent),'.mat']);
end